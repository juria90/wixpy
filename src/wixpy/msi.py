...
def msi_str(text):
    if not utils.IS_PY3:
        text = text.decode("utf-8").encode("cp%s" % MSI_CODEPAGE, errors="replace")
    return text
...
class InstallMode(object):
    ...
    def from_string(cls, str_value):
        return {"install": cls.INSTALL, "uninstall": cls.UNINSTALL, "both": cls.BOTH}[
            str_value.lower()
        ]
...
MSI_ACTIONS = {
    "InstallInitialize": (
        None,
        1500,
        ActionFlags.ADMIN_EXECUTE_SEQUENCE
    ...
    "ExecuteAction": (
        None,
        1300,
        ActionFlags.ADMIN_UI_SEQUENCE | ActionFlags.INSTALL_UI_SEQUENCE,
    ),
    "CreateShortcuts": (
        None,
        4500,
        ActionFlags.ADVT_EXECUTE_SEQUENCE | ActionFlags.INSTALL_EXECUTE_SEQUENCE,
    ),
    ...
    "RemoveEnvironmentStrings": (None, 3300, ActionFlags.INSTALL_EXECUTE_SEQUENCE),
    ...
    "MsiUnpublishAssemblies": (None, 1750, ActionFlags.INSTALL_EXECUTE_SEQUENCE),
    ...
    "UnregisterExtensionInfo": (None, 2800, ActionFlags.INSTALL_EXECUTE_SEQUENCE),
    ...
    "UnregisterTypeLibraries": (None, 2300, ActionFlags.INSTALL_EXECUTE_SEQUENCE),
    "ValidateProductID": (
        None,
        700,
        ActionFlags.INSTALL_EXECUTE_SEQUENCE | ActionFlags.INSTALL_UI_SEQUENCE,
    ),
    "WriteEnvironmentStrings": (None, 5200, ActionFlags.INSTALL_EXECUTE_SEQUENCE),
...
MT_TABLES = {
    ...
    MT_LAUNCHCONDITION: (
        ...
        ("Description", "CHAR(255) NOT NULL " "LOCALIZABLE PRIMARY KEY `Condition`"),
    ),
    MT_APPSEARCH: (
        ("Property", "CHAR(72) NOT NULL"),
    ...
    MT_CREATEFOLDER: (
        ("Directory_", "CHAR(72) NOT NULL"),
        ("Component_", "CHAR(72) NOT NULL " "PRIMARY KEY `Directory_`, `Component_`"),
    ),
    MT_SIGNATURE: (
    ...
    MT_ENVIRONMENT: (
        ...
        ("Name", "CHAR(255) NOT NULL"),
        ("Value", "CHAR(255)"),
        ("Component_", "CHAR(72) NOT NULL " "PRIMARY KEY `Environment`"),
    ),
}
...
class MsiDatabase(libmsi.Database):
    ...
    def __init__(self, model):
        ...
        self.model = model
        self.tables = {key: MsiTable(key) for key in list(MT_TABLES.keys())}
    ...
    def set_action_sequences(self):
        ...
        if not self.tables[MT_FILE].records and self.tables[MT_REMOVEFILE].records:
            tb.add_action("RemoveFiles")
    ...
    def write_msi(self, msifile):
        ...
        for record in validate.RECORDS:
            if record[0] in (MT_VALIDATION, MT_STREAMS) or (
                record[0] in self.tables and self.tables[record[0]].records
            ):
