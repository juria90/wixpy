...
ATTRS = {
    ...
    "Environment": ("Id", "Name", "Value", "Permanent", "Part", "Action", "System"),
}
...
class WixElement(object):
    ...
    def __init__(self, **kwargs):
        self.childs = []
        self.attrs = {
            key: value for key, value in list(kwargs.items()) if key in ATTRS[self.tag]
        }
...
class WixCondition(WixElement):
    ...
    def write_msi_records(self, db):
        db.tables[msi.MT_LAUNCHCONDITION].add(self.condition, self.get("Message"))
...
class WixOsCondition(WixCondition):
    def __init__(self, os_condition):
        ...
        os_condition = (
            "501" if str(os_condition) not in OS_CONDITION else str(os_condition)
        )
        msg = (
        ...
        super(WixOsCondition, self).__init__(msg, os_condition, comment=comment)
...
class WixArchCondition(WixCondition):
    def __init__(self):
        ...
        msg = (
            "64-bit operating system was not detected, "
            "please use the 32-bit installer."
        )
        super(WixArchCondition, self).__init__(msg, "VersionNT64", comment=comment)
...
class WixIcon(WixElement):
    ...
    def __init__(self, source):
        super(WixIcon, self).__init__(SourceFile=source, Id=os.path.basename(source))
...
class WixMedia(WixElement):
    ...
    def write_msi_records(self, db):
        cabinet = (
            "#%s" % self.get("Cabinet")
            if self.get("EmbedCab") == "yes"
            else self.get("Cabinet")
        )
        disk = int(self.get("Id"))
        rec = db.tables[msi.MT_MEDIA].add(
            disk, 0, self.get("DiskPrompt"), cabinet, None, None
        )
...
class WixFile(WixElement):
    ...
    def get_msi_name(self):
        ...
        return "|".join([shortname, filename]) if shortname != filename else filename
    ...
    def write_msi_records(self, db):
        ...
        table.add(
            file_id, comp_id, name, size, None, None, msi.FileAttribute.VITAL, sequence
        )
...
class WixComponent(WixElement):
    ...
    def __init__(self, path=None, rel_path=None, **data):
        super(WixComponent, self).__init__(Guid=utils.get_guid(), **data)
...
class WixDirectory(WixElement):
    ...
    def __init__(self, data=None, path=None, rel_path=None, **kwargs):
        name = kwargs["Name"] if "Name" in kwargs else os.path.basename(rel_path)
    ...
    def get_msi_name(self, dirname):
        ...
        ext = ext[:3] if ext and len(ext) > 3 else ext
        shortname = ".".join([name, ext]) if ext else name
        return "|".join([shortname, dirname]) if shortname != dirname else dirname
...
class WixInstallDir(WixElement):
    ...
    def __init__(self, data):
        super(WixInstallDir, self).__init__(
            Id="INSTALLDIR", Name=data.get("_InstallDir")
        )
...
class WixFeature(WixElement):
    ...
    def write_msi_records(self, db):
        ...
        table.add(
            ...
            msi.FeatureDisplay.COLLAPSE,
            int(self.get("Level")),
            self.get("ConfigurableDirectory"),
            0,
        )
...
class WixShortcutComponent(WixComponent):
    ...
    def __init__(self, data, shortcut_data):
        ...
        self.add(WixRemoveFolder(Id=shortcut_data["DirectoryRef"], On="uninstall"))
...
class WixEnvironment(WixElement):
    ...
    def write_msi_records(self, db):
        ...
        if self.get("Part") == "last":
            ...
            value = "%s;[~]" % self.get("Value")
        table.add(
            self.get("Id"), name_prefix + self.get("Name"), value, self.parent.get("Id")
        )
...
class WixProduct(WixElement):
    ...
    def set_shortcuts(self, data, target_dir):
        if data.get("_Shortcuts") and data.get("_ProgramMenuFolder"):
            ...
            shortcut_dir = WixDirectory(
                Id=utils.get_id("mnu"), Name=data.get("_ProgramMenuFolder")
            )
            ...
            for shortcut in data.get("_Shortcuts"):
                ...
                if shortcut.get("AddOnDesktop"):
                    ...
                    if desktop_dir is None:
                        desktop_dir = WixDirectory(Id="DesktopFolder", Name="Desktop")
                ...
                if shortcut.get("OpenWith"):
                    ...
                    component.add(
                        WixRegistryValue(Root="HKCR", Key=target_ref, Value=description)
                    )
                    # Shortcut open option
                    key = target_ref + "\\shell\\open"
                    value = "Open with %s" % shortcut["Name"]
                    component.add(WixRegistryValue(Root="HKCR", Key=key, Value=value))
                    ...
                    for item in shortcut.get("OpenWith"):
                        ...
                        component.add(
                            WixRegistryValue(Root="HKCR", Key=key, Name=target_ref)
                        )
                for item in shortcut.get("Open", []):
                    ...
                    if item.get("EditWith"):
                        ...
                        component.add(
                            WixRegistryValue(Root="HKCR", Key=key, Value=value)
                        )
                    ...
                    # File association
                    component.add(
                        WixRegistryValue(Root="HKCR", Key=ext, Value=target_ref)
                    )
                    component.add(
                        WixRegistryValue(
                            Root="HKCR", Key=ext, Name="Content Type", Value=mime
                        )
                    )
...
class Wix(WixElement):
    ...
    def __init__(self, data):
        ...
        self.comment = "Generated by %s %s" % (data["_pkgname"], data["_pkgver"])
