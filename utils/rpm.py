...
class RpmBuilder(object):
    ...
    def __init__(
        ...
        self.spec_path = os.path.join(
            self.rpmbuild_path, "SPECS", "%s.spec" % self.name
        )
    ...
    def write_spec(self):
        ...
        content += [
            ...
            "",
            "%prep",
            "%autosetup -n {}-{}".format(self.name, self.version),
            "",
            "%build",
            "/usr/bin/python2 %s build" % self.build_script,
            "",
            "%install",
    ...
    def build_rpm(self):
        os.system(
            'rpmbuild -bb %s --define "_topdir %s"'
            % (self.spec_path, self.rpmbuild_path)
        )
        os.system(
            'cp `find %s -name "*.rpm"` %s/' % (self.rpmbuild_path, self.dist_dir)
        )
